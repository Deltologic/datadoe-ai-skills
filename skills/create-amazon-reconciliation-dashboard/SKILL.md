...
| Status       | orders      | `amazon_order_status` (Shipped / Cancelled / Pending)                  |
| Channel      | orders      | `fulfillment_channel` (Amazon / Merchant)                              |
| B2B          | orders      | `order_is_business` (Yes/No badge)                                     |
| Items        | orders      | sum of `quantity` per `amazon_order_id`                                |
| Revenue      | orders      | sum of `item_price_value` per order, £ (use ONLY this for order value) |
| Tax          | orders      | sum of `item_tax_value` per order, formatted as £                      |
...
| Settled £    | settlements | settlement `item_price`                                                |
| Fees         | settlements | `referral_fee` + FBA fees (sum the `fba_*` columns, or `total_fees`); negative |
| Refund £     | settlements | `refunded_amount` (if any; negative)                                   |
| Net Payout   | settlements | settlement `total` (may be negative)                                   |
...
- Find and save the source IDs for:
  - **"Order Line Items"** (source id `89b27535d2`, table `amazon_order_items_with_cogs`) -
    **line-item grain: one row per order item, NOT pre-aggregated per order.**
  - **"Settlements & P&L Components"** (source id `732dac689a`, table `amazon_settlements_with_cogs`).
...
- **Order Line Items are line-item grain** (one row per item): group by `amazon_order_id`
  and sum `quantity` / `item_price_value` / `item_tax_value` to get order-level values.
  Also deduplicate by `amazon_order_id` across overlapping batches.
...
- Calculate daily shipped revenue (sum of `item_price_value`) and tax (sum of `item_tax_value`).
- Track Amazon vs Merchant `fulfillment_channel` breakdown.

Settlements — group by date, separated by `settlement_type`:

- `ORDER` entries: count, `item_price`, `item_tax`, `referral_fee`, FBA fees (sum the `fba_*` columns, or `total_fees` all-in), `total`. (Fees are negative.)
...
4. Assign a reconciliation status:
   ...
   - **Cancelled** → `amazon_order_status` is `Cancelled` (no settlement expected).
...
- Total fees breakdown: referral (`referral_fee`), FBA (sum the `fba_*` columns), other (`total_fees` minus referral and FBA). Fees are negative — normalize signs.
...
**Order Line Items (source `89b27535d2`, table `amazon_order_items_with_cogs`) — LINE-ITEM grain; aggregate client-side per `amazon_order_id`:**

| Field                  | Role        | Notes                                                                                  |
| ---------------------- | ----------- | -------------------------------------------------------------------------------------- |
| `amazon_order_id`      | Key         | One order = many line rows; group + sum by this                                        |
| `date`                 | Order       | Prefer for from/to filtering (`order_date` is a duplicate)                             |
| `order_date`           | Order       | Purchase date for "Order Month" assignment (duplicate of `date`)                       |
| `amazon_order_status`  | Order       | `Pending` / `Shipped` / `Cancelled`                                                    |
| `item_status`          | Order       | Finer status `Unshipped`/`Shipping`/`Shipped`/`Canceled` (optional)                    |
| `fulfillment_channel`  | Order       | `Amazon` / `Merchant`                                                                   |
| `sales_channel`        | Order       | Flags MCF orders (optional)                                                            |
| `order_is_business`    | Order       | Boolean → "Yes" / "No" badge                                                           |
| `quantity`             | Line item   | Units on this line — **sum per `amazon_order_id`**                                     |
| `item_price_value`     | Line item   | **Use ONLY this for order value** — sum per `amazon_order_id`                          |
| `item_tax_value`       | Line item   | Order tax — sum per `amazon_order_id`                                                  |
| `child_asin` / `sku` / `product_name` / `cogs_total_value` | Line item | Optional per-line detail                            |

**Settlements & P&L Components (source `732dac689a`, table `amazon_settlements_with_cogs`) — expenses are NEGATIVE values:**

| Field                  | Role        | Notes                                                                                  |
| ---------------------- | ----------- | -------------------------------------------------------------------------------------- |
| `amazon_order_id`      | Join key    | Matches the orders table; nullable for non-order settlement rows                       |
| `date`                 | Settlement  | Posted-date (when Amazon processed the settlement); use for from/to filtering          |
| `order_date`           | Settlement  | Purchase date (up to ~30 days before `date`) — the cross-month reconciliation key      |
| `settlement_type`      | Settlement  | `ORDER` / `REFUND` / `OTHER`                                                           |
| `item_price`           | Amount      | Settled revenue for the row                                                            |
| `item_tax`             | Amount      | Settled tax                                                                            |
| `referral_fee`         | Fee         | Amazon referral fee (negative)                                                         |
| `fba_*` (e.g. `fba_per_unit_fulfillment_fee`, `fba_storage_fee`, `fba_transportation_fee`) | Fee | FBA fees are **split across several columns** — no single `fba_fee`; sum them, or use `total_fees` |
| `total_fees`           | Fee         | All-in fees for the row (negative) — convenient waterfall input                        |
| `refunded_amount`      | Amount      | Refund amount (only on `REFUND` rows; negative)                                        |
| `total`                | Amount      | Net amount for the row; **may be negative**                                            |

**Sign convention:** expenses (fees, refunds) are stored **negative**. Standardize signs before comparing with the orders table or building the Referral / FBA / Other waterfall (take absolute values for fee magnitudes; let `total` keep its sign for net payout).
...
- **`columns: []` (empty array) does NOT return all columns.** It returns only implicit seller-context metadata (`seller_id`, `seller_name`, `marketplace_name`, etc.) and none of the table fields. **Always specify the required columns explicitly.** DataDoe prepends 8 utility columns (`seller_id` … `marketplace_name`) before your requested fields, so **access every field by name, never by position**.
