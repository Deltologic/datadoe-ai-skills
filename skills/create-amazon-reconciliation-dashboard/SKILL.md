...
| Order Date   | orders      | `date` (purchase date in the marketplace timezone)                     |
| Order Month  | computed    | derived from orders `date`, e.g. "Feb 2026"                            |
| Status       | orders      | `amazon_order_status` (Shipped / Canceled / Pending)                   |
| Channel      | orders      | `fulfillment_channel` (Amazon / Merchant)                              |
| B2B          | orders      | `order_is_business` (Yes/No badge)                                     |
| Items        | orders      | sum of `quantity`                                                      |
| Revenue      | orders      | sum of `item_price_value` formatted as £                               |
| Tax          | orders      | sum of `item_tax_value` formatted as £                                 |
...
| Settled £    | settlements | sum of settlement `item_price`                                         |
| Fees         | settlements | sum of `referral_fee` + `fba_per_unit_fulfillment_fee`                 |
| Refund £     | settlements | sum of `refunded_amount` (REFUND rows)                                 |
| Net Payout   | settlements | sum of settlement `total`                                              |
...
**1. Review the DataDoe usage instructions.**

- The DataDoe MCP server sends its usage instructions with the connection, so you already have them when the session starts. They describe seller/vendor scoping, rate limits, and required fields — do not skip them.
- If you need more detail on a specific feature, call `datadoe_user_docs_table_of_contents_get` to list the documentation pages, then `datadoe_user_docs_page_get` to read one.
...
- Find and save the source IDs for:
  - **"Order Line Items"** (table: `amazon_order_items_with_cogs`).
  - **"Settlements & P&L Components"** (table: `amazon_settlements_with_cogs`).
...
- `ORDER` entries: count, `item_price`, `item_tax`, `referral_fee`, `fba_per_unit_fulfillment_fee`, `total`.
...
1. Record which month the order belongs to (by orders `date`).
2. Look up whether it has a matching settlement entry (by `amazon_order_id` in settlement exports).
3. Note that the settlement `date` (posted date) may be in a **different month** than the order `date` — this is normal and critical for cross-month reconciliation. Settlements also carry `order_date` for the original purchase date.
...
5. Calculate per-order:
   - Order side: revenue (sum of `item_price_value`), tax (sum of `item_tax_value`), quantity (sum of `quantity`).
...
| `item_price_value`     | Line item   | Line revenue. Excludes buyer shipping. Sum per order.                                 |
...
| `date`                            | Settlement  | Posted date. Export from/to filters use this column.                        |
...
| `fba_per_unit_fulfillment_fee`    | Amount      | FBA per-unit fulfilment fee                                                  |
...
- **`columns: []` (empty array) does NOT return all columns.** It returns only seller-context columns (`seller_or_vendor_id`, `seller_or_vendor_name`, `marketplace_country_name`, `marketplace_seller_id`) and none of the table fields. **Always specify the required columns explicitly.**
...
| ----------------------------------------------------------------- | -------------------------------------------------------------- |
| `datadoe_user_docs_page_get`                                       | `GET /api/v1/spec/datadoe_api.md`                              |
