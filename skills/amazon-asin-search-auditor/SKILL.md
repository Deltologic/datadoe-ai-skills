...
9. Search for each unique search term and collect all ASINs with their organic/paid search ranks:
   ...
   ```
10. (Spawn a subagent for this step) Extract unique ASINs from all results for each search term using `scripts/list-asins.js`.
11. (Spawn a subagent for this step) Fetch active listings for the seller using DataDoe and save it to CSV (include only ASINs):
    - Source: `amazon_listings_with_cogs`.
    - Paginate if the seller has more listings than the Export limit.
12. (Spawn a subagent for these steps) Run the file-based post-processor for the step 11 ASIN list, step 12 listings export, and the dashboard build:
    - Mark my listings in JSONs using `scripts/mark-my-listings.js`.
    - Generate `report-data.json`.
    - Render `report.html`.
    - Run contract checks.
    - Return only summary stats + paths.
13. That's all. Open the report page for the user.
